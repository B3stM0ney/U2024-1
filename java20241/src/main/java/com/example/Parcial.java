...
public class Parcial {
    public static void main(String[] args) {
        ...
        String[] nombresJockeys = { "Ana", "Pedro", "Juan", "Cesar", "Diana", "Maria" }; // lista de los jockeys que
                                                                                         // participan//

        int[] resultadosCarreras = new int[CarrerasMax * DatosCarrera]; // arreglo para guardar los resultados de todas
                                                                        // las carreras//
        ...
        for (int i = 0; i < resultadosCarreras.length; i += DatosCarrera) {
            ...
            resultadosCarreras[i + 1] = random.nextInt(100) + 100; // Tiempo del ganador (un numero entre 0 y 100 + 100
                                                                   // para que el resultado sea un numero aleatorio
                                                                   // entre 100 y 199 segundos)
        ...
        for (int i = 0; i < CarrerasMax; i++) {
            System.out.println("No. Carrera " + i + ", Ganador: Código " + resultadosCarreras[i * DatosCarrera] + " ("
                    + nombresJockeys[resultadosCarreras[i * DatosCarrera]] + "), Tiempo Ganador: "
                    + resultadosCarreras[i * DatosCarrera + 1]
                    + ", Segundo: Código " + resultadosCarreras[i * DatosCarrera + 2] + " ("
                    + nombresJockeys[resultadosCarreras[i * DatosCarrera + 2]] + "), Tiempo Segundo: "
                    + resultadosCarreras[i * DatosCarrera + 3]
                    + ", Tercero: Código " + resultadosCarreras[i * DatosCarrera + 4] + " ("
                    + nombresJockeys[resultadosCarreras[i * DatosCarrera + 4]] + "), Tiempo Tercero: "
                    + resultadosCarreras[i * DatosCarrera + 5]);

            // resultadosCarreras[i*DatosCarrera] obtiene el indice o cidgo del jockey
            // ganador de la carrera almacenado en "resultadosCarreras" que contiene los
            // resultados
            // de todas las carreras por lo que i*datoscarrera calcula el inicio
            // correspondiente a la carrera actual
            // nombresJockeys[resultadosCarreras[i*DatosCarrera]] utiliza el codigo del paso
            // anterior como indice para acceder al arreglo nombresJockey y recupera
            // el nombre del jockey ganador
            // para el segundo y tercer lugar se repite el proceso acciendo los codigos de
            // cada jockey y tiempos de los resultados de las carreras.
        ...
        for (int i = 0; i < 3; i++) {
            ...
            System.out.println((i + 1) + "º lugar: " + nombresJockeys[indiceJockeyActual] + " con "
                    + puntuacionJockey[indiceJockeyActual] + " puntos");
        }
